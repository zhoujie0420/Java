...
public class InteractionReplyServiceImpl extends ServiceImpl<InteractionReplyMapper, InteractionReply> implements IInteractionReplyService {
    ...
    public PageDTO<ReplyVO> queryReplyPage(ReplyPageQuery query, boolean forAdmin) {
        ...
        // 3.数据处理，查询：提问者信息、回复目标信息、当前用户是否点过赞
        ...
        for (InteractionReply r : records) {
            if (forAdmin || Boolean.FALSE.equals(r.getAnonymity())) {
                // 非隐藏或是管理员，直接添加用户进去
        ...
        // 3.2 查询目标回复，如果回复目标不是匿名则需要查询用户信息
        ...
        for (InteractionReply r : records) {
            ...
            // 4.3 如果存在评论的目标，需要设置目标用户信息
            if (Objects.nonNull(r.getTargetReplyId())) {
